...
public class BellsPizzaSimulation {
    ...
    private Map<String, Integer> destinationToCostMap = new HashMap<>();
    ...
    private void loadDestinations() {
        try {
            ...
            while (sc.hasNext()) {
                destinationToCostMap.put(sc.next(), sc.nextInt());
            }
    ...
    private void runTests() {
        try {
            for (int i = 1; i <= 20; i++) {
                ...
                double profitQueue = run(new LinkedList<>(), new Scanner(new File(fname)));

                System.out.print("PriorityQueue:  ");
                double profitPQueue = run(new HeapArrayQueue<>(), new Scanner(new File(fname)));
    ...
    private double run(Queue<Pizza> queue, Scanner logFile) {
        ...
        int numCyclist = 3;
        int[] cyclistAvailableAt = new int[numCyclist];     // the time when the cyclist will next be available.
        ...
        while (logFile.hasNext()) {
            ...
            for (int i = 0; i < numCyclist; i++) {
                if (time > cyclistAvailableAt[i] && !queue.isEmpty()) {
                    ...
                    cyclistAvailableAt[i] = time + fullTripTime;
    ...
    // run the Bells Pizza simulation
